    ...
    <form action="{{ route('simats.update', $simat->id) }}" method="POST" id="simat-form">
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-2">
                ...
                <input type="text" name="mother_name" value="{{ old('mother_name', $simat->mother_name) }}" class="form-control">
            </div>
            <div class="col-md-4 mb-2">
                <label>تاريخ الميلاد:</label>
                <input type="date" name="birth_date" value="{{ old('birth_date', $simat->birth_date) }}" class="form-control">
            </div>

            <div class="col-md-4 mb-2">
                <label>رقم الجواز:</label>
                <input type="text" name="passport_number" value="{{ old('passport_number', $simat->passport_number) }}" class="form-control" required>
            </div>
            <div class="col-md-4 mb-2">
                <label>تاريخ الدخول:</label>
                <input type="date" name="entry_date" value="{{ old('entry_date', $simat->entry_date) }}" class="form-control" required>
            </div>

            <div class="col-md-4 mb-2 position-relative">
                <label>الجنسية:</label>
                <input type="text" id="nationalitySearchInput" class="form-control" placeholder="ابحث عن الجنسية..." autocomplete="off"
                    value="{{ old('nationality', $simat->nationality) }}">

                <!-- نرسل الاسم فقط -->
                <input type="hidden" name="nationality" id="selectedNationalityName" value="{{ old('nationality', $simat->nationality) }}">
            ...
            <div class="col-md-4 mb-2">
                ...
                <select name="visa_type" id="visa_type" class="form-control" required>
                    ...
                    <option value="مرور" {{ old('visa_type', $simat->visa_type) == 'مرور' ? 'selected' : '' }}>مرور</option>
                    <option value="دخول" {{ old('visa_type', $simat->visa_type) == 'دخول' ? 'selected' : '' }}>دخول</option>
                </select>
            </div>
            ...
            <div class="col-md-4 mb-2">
                <label>الرسوم رقمياً:</label>
                <input type="number" name="fee_number" value="{{ old('fee_number', $simat->fee_number) }}" class="form-control" required>
            </div>

            <div class="col-md-4 mb-2">
                <label>الرسوم كتابة:</label>
                <input type="text" name="fee_text" value="{{ old('fee_text', $simat->fee_text) }}" class="form-control" required>
            </div>

            <div class="col-md-4 mb-2">
                <label>رمز الدولة:</label>
                <input type="text" name="country_code" value="{{ old('country_code', $simat->country_code) }}" class="form-control">
            </div>

            <div class="col-md-4 mb-2">
                <label>رسوم عمالة:</label>
                <input type="number" name="labor_fee" value="{{ old('labor_fee', $simat->labor_fee) }}" class="form-control">
            </div>
        </div>
